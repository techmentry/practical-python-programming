...
What this means is that whatever I type between the quoted 
section no matter how many lines are used is a comment.
"""

'''
This is also called a multi-line comment in python.
There is no difference between writing a multi-line comment using 3 double-quotes(")or 3 single quotes (').
'''

"""
In python, to print something to the console/ result window/ output window, we can use print.
print is a function.
"""
print("Hello world!")

"""
What is a function? - Functions are a set or group of instructions.
...
2. Function parameters - allow you to give them (called as passing) values
                            which it can use and do something (Optional)
3. Function return value - A function can have a return value
                            based on what the function does (Optional)
"""

"""
Variables
Are quantities whose values are not fixed, because variables are used
to create a generalized logic or algorithm in such a way that no matter
what value is present in that variable, the output can be determined.
"""

"""
In programming languages, variables have a data-type
Examples of data types - integer, floating point values, strings, char,

What does a data-type mean?
    What a data type is telling you is what kind of data does a variable store.
    And how should the computer deal with it
...
------------------------------------
For the computer, the binary number (01011001 or 0x59 or 89 in decimal) can mean anything.
* if you read it as an int, then it will give me 89
* if I tell the computer to read this as a character, then it will read it as 'Y'
* if I tell the computer to read this as something else,
    then it will it read it as that something which is defined concretely

-- The way you know to read this as
alphabet Y
or number
or something else

is through the data type of the variable
"""

'''
Rules for defining a variable
1. You can only use numbers, alphabets, and underscore in your variable name.
    All other characters like #,$,- are not allowed.
2. Your variable name needs to start with either an alphabet or an underscore.
    e.g. a, a1, b2, _a, _1 are valid. 1a, 1_a 1_2, 1_ are not valid.
3. You need to have at least 1 character as a part of your variable name.
    e.g. a aa aaaaaaa aaaa_aaaa _aasdfadsfasd adfasdfasdf_ _adsfadsf23423_asdfads_asdf_123 are valid
4. Variable names are case sensitive. 
    i.e. var1, Var1, vAr1, vaR1 are all DIFFERENT variables. They are not the same.
'''

'''
We will use Snake case (https://en.wikipedia.org/wiki/Snake_case) for our naming programming style.
'''
...
# examples of performing operations with variables
int_var_1 = 22
int_var_2 = 56
sum_var = int_var_1 + int_var_2 + 5 + int_sum
print("My complex integer sum is: " + str(sum_var))
print("This is an example of directly printing the sum of a variable without storing it in another variable: " +
      str(int_var_1 + int_var_2 + 5 + int_sum))
...
'''
How to convert from a data type to another data type
'''
...
print(float(str(2)))  # Conversion order str(2) -> float('2') -> 2.0

# The following will give you an error because str(2.35) gives '2.35'
# and int cannot convert any non-number characters.
# print(int(str(2.35)))
...
'''
function evaluation order - inside out. 
e.g. print(float(str(55.6))) will first evaluate str(55.6) as "55.6",
then this will be passed to float("55.6") which will then
return 55.6 which is passed to print function.
'''
